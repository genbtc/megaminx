...
void Megaminx::rotateSolveLayer7Edges(Megaminx* shadowDom)
{
    ...
    do {
        ...
        bool twoAdjacentColors = (
            (!grayFaceColorSolved[0] && !grayFaceColorSolved[1]) ||
            (!grayFaceColorSolved[1] && !grayFaceColorSolved[2]) ||
            (!grayFaceColorSolved[2] && !grayFaceColorSolved[3]) ||
            (!grayFaceColorSolved[3] && !grayFaceColorSolved[4]) ||
            (!grayFaceColorSolved[4] && !grayFaceColorSolved[0]));
        bool twoOppositeColors = (
            (!grayFaceColorSolved[2] && !grayFaceColorSolved[4]) ||
            (!grayFaceColorSolved[3] && !grayFaceColorSolved[0]) ||
            (!grayFaceColorSolved[4] && !grayFaceColorSolved[1]) ||
            (!grayFaceColorSolved[0] && !grayFaceColorSolved[2]) ||
            (!grayFaceColorSolved[1] && !grayFaceColorSolved[3]));
        auto twoGraysUnsolved = [grayFaceColorSolved](int a, int b) { return (!grayFaceColorSolved[a] && !grayFaceColorSolved[b]); };
        auto matchesOppoColor = [grayFaceColorSolved](int face) {
            face -= LIGHT_BLUE;
            int a = face + 2;
            int b = a + 2;
            MM(a);
            MM(b);
            return (!grayFaceColorSolved[a] && !grayFaceColorSolved[b]);
        };
        ...
        bool allCornersAlreadySolved = std::all_of(std::begin(grayCornerColorSolved), std::end(grayCornerColorSolved), [](bool j) { return j; });
//START MAIN:

        //orient the first piece if it exists:
        int findIfPieceSolved = shadowDom->findEdge(25); //always piece 0
        if (findIfPieceSolved > 25 && findIfPieceSolved < 30 && !allCornersAlreadySolved && !piecesSolvedStrict[0] && offby != 0 && !twoAdjacentPieces) {
            //Test 3 passes.
        ...
        else if (!piecesSolvedStrict[0] && !twoAdjacentPieces && !allCornersAlreadySolved && firstSolvedPiece != -1 && piecesSolvedMaybe[i - 25] && solvedCount >= (i - 25)) {
...
////5-way cycle -2
        else if (checkPieceMatches(pieceOrder, 28, 29, 25, 26, 27) && allEdgeColorsSolved && allCornersAlreadySolved) //TT55-3
        {
            //This cuts previous algo17Blue down by 12 moves: (called on Beige, Light_Blue, Orange) faces
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[40].algo, g_faceNeighbors[ORANGE]);
        }
//5-way cycle +2
        else if (checkPieceMatches(pieceOrder, 27, 28, 29, 25, 26) && allEdgeColorsSolved && allCornersAlreadySolved) //TT53-2
        {
            //This cuts previous algo38Blue down by 12 moves: (called on Green, Orange, Light_Blue) faces
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[41].algo, g_faceNeighbors[LIGHT_GREEN]);
        }
//TODO: Combine
//5-way cycle (+1,+2,-1,+2,+1)
        else if (checkPieceMatches(pieceOrder, 26, 28, 29, 27, 25) && allEdgeColorsSolved && allCornersAlreadySolved) //TT56-2
        {
            //Streamlined previous algo18 from 60 moves to 32 :
            //bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[18].algo, g_faceNeighbors[LIGHT_BLUE]);
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[39].algo, g_faceNeighbors[PINK]);
        }
//5-way cycle (+1,+2,-1,+2,+1)
        else if (checkPieceMatches(pieceOrder, 26, 27, 29, 25, 28) && allEdgeColorsSolved && allCornersAlreadySolved) //TT57-2
        {
            //Streamlined previous algo18 from 60 moves to 32 :
            //bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[18].algo, g_faceNeighbors[ORANGE]);
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[39].algo, g_faceNeighbors[BEIGE]);
        }
//5-way cycle (+1,+2,-1,+2,+1)
        else if (checkPieceMatches(pieceOrder, 29, 27, 28, 25, 26) && allEdgeColorsSolved && allCornersAlreadySolved) //TT54-2
        {
            //Streamlined previous algo18 from 60 moves to 32 :
            //bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[18].algo, g_faceNeighbors[LIGHT_GREEN]);
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[39].algo, g_faceNeighbors[LIGHT_BLUE]);
        }
//5-way cycle (+1,+2,-1,+2,+1)
        else if (checkPieceMatches(pieceOrder, 27, 25, 28, 29, 26) && allEdgeColorsSolved && allCornersAlreadySolved) //TT58-2
        {
            //Streamlined previous algo18 from 60 moves to 32 :
            //bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[18].algo, g_faceNeighbors[PINK]);
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[39].algo, g_faceNeighbors[ORANGE]);
        }
//5-way cycle (+1,+2,-1,+2,+1)
        else if (checkPieceMatches(pieceOrder, 27, 28, 26, 29, 25) && allEdgeColorsSolved && allCornersAlreadySolved) //TT59-2
        {
            //Streamlined previous algo18 from 60 moves to 32 :
            //bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[18].algo, g_faceNeighbors[BEIGE]);
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[39].algo, g_faceNeighbors[LIGHT_GREEN]);
        }

//BUNNY 4-COLOR
        else if (offby == 0 && solvedCount == 1 && !allEdgeColorsSolved & allCornersAlreadySolved)
        {
        ...
        else if (offby == 0 && solvedCount == 3 && !allEdgeColorsSolved && (twoGraysUnsolved(2,3) || twoGraysUnsolved(1,2)))
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[29].algo, g_faceNeighbors[LIGHT_BLUE]);    //algo #29 Quad-Flip Colors
        }
//BUNNY 2-COLOR
...
//2&5 + 3&4 (opposite across)
        else if (offby == 3 && solvedCount == 1 && piecesSolvedStrict[4] && pieceOrder[0] == 28 && pieceOrder[1] == 27 && pieceOrder[2] == 26) //TT46-2
        {
...
//MUSHROOM- //algo #14 3a-  (F/L Safe)
        else if (offby == 3 && solvedCount == 1 && piecesSolvedStrict[0] && twoGraysUnsolved(3,4)) {   //TestCube22-2Edges
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(0,1) && twoGraysUnsolved(3,4))
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        ...
        //first and second piece solved for sure, attached
            ...
            (solvedCount >= 2 && twoAdjacentPieces && (twoGraysUnsolved(1,2) || twoGraysUnsolved(2,3) || allEdgeColorsSolved)))  //test5
        ) {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);  //algo #14 3a-  (F/L Safe)
            //Test2-pt2-pass      //Test4-fixns    //test5
        }
        else if (offby == 2 && solvedCount == 1 && allEdgeColorsSolved)
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);  //algo #14 3a-  (F/L Safe)
        }
        else if ((offby == 1 && !currentpieceFlipStatusOK || offby == 4 && currentpieceFlipStatusOK) && solvedCount == 2 && twoSolvedPieces(2,3) && grayFaceColorSolved[4] && twoGraysUnsolved(0, 1)) { //TT9
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);  //algo #14 3a-  (F/L Safe)
        }
        else if (offby == 2 && solvedCount == 2 && twoAdjacentPieces && !currentpieceFlipStatusOK && twoGraysUnsolved(3,4) && twoSolvedPieces(0,1)) //TT11
        { 
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);  //algo #14 3a-  (F/L Safe)
        }
        else if (offby == 4 && solvedCount == 2 && twoAdjacentPieces && allEdgeColorsSolved && twoSolvedPieces(1,2)) //TT14
        { 
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && !currentpieceFlipStatusOK && twoSolvedPieces(0,4) && twoGraysUnsolved(2,3)) //TT15
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 1 && solvedCount == 1 && !currentpieceFlipStatusOK && piecesSolvedStrict[0] && twoGraysUnsolved(2,3)) //TT20-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 4 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(2,3)) //TT27-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && !currentpieceFlipStatusOK && twoSolvedPieces(3,4) && twoGraysUnsolved(1,2)) //TT29-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 3 && solvedCount == 0 && allCornersAlreadySolved && !piecesSolvedStrict[0] && twoGraysUnsolved(1,2)) //TT35-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + offby]);
        }
        else if (offby == 2 && solvedCount == 0 && allCornersAlreadySolved && allEdgeColorsSolved)  //TT38-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[BEIGE]);
        }
        else if (offby == 3 && solvedCount == 1 && allCornersAlreadySolved && allEdgeColorsSolved && (piecesSolvedStrict[1] || piecesSolvedStrict[2]) ) //TT37-1 //TT42-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]);
        }
        else if (offby == 2 && solvedCount == 1 && allCornersAlreadySolved && piecesSolvedStrict[1] && twoGraysUnsolved(2,3)) //TT43-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[BEIGE]);
        }
        else if (offby == 4 && solvedCount == 0 && allCornersAlreadySolved && !currentpieceFlipStatusOK && twoGraysUnsolved(4,0)) //TT45-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[BEIGE]);
        }
        else if (offby == 4 && solvedCount == 0 && allCornersAlreadySolved && twoGraysUnsolved(2,3)) //TT48-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[BEIGE]);
        }
        else if (offby == 4 && solvedCount == 0 && allCornersAlreadySolved && pieceOrder[0] == 26) //TT50-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[PINK]);
        }
        else if (offby == 3 && solvedCount == 1 && piecesSolvedStrict[1] && allCornersAlreadySolved && twoGraysUnsolved(3,4) && pieceOrder[0] == 28) //TT51-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[PINK]);
        }
        else if (offby == 2 && solvedCount == 0 && allCornersAlreadySolved && twoGraysUnsolved(4,0) && pieceOrder[0] == 26) //TT52-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_GREEN]);
        }
        else if (offby == 1 && solvedCount == 0 && allCornersAlreadySolved && twoGraysUnsolved(3,4) && pieceOrder[0] == 28) //TT60-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[14].algo, g_faceNeighbors[LIGHT_BLUE]);
        }
...
//MUSHROOM+
        ...
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(0,1) && (allEdgeColorsSolved || twoGraysUnsolved(2, 3)))  //test4-p2 ->(blue/orange)=PASS
        {                                                                                                                                       //TestCube6-2
        ...
        else if (offby == 3 && solvedCount == 2 && twoSolvedPieces(1,2))  //test3-pt2-manual->(orange/green)=PASS
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 1 && piecesSolvedStrict[0] && twoGraysUnsolved(3,4) && grayFaceColorSolved[1] && grayFaceColorSolved[2])
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 1 && piecesSolvedStrict[0] && !currentpieceFlipStatusOK && twoGraysUnsolved(1,2) ) //TT13
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (solvedCount == 1 && piecesSolvedStrict[0] && !currentpieceFlipStatusOK && twoGraysUnsolved(1,2)) //TT19-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(4,0) && !currentpieceFlipStatusOK && twoGraysUnsolved(1,2)) //TT18-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 4 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(1,2) && !currentpieceFlipStatusOK && twoGraysUnsolved(3,4)) //TT17-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 4 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(2,3) && !currentpieceFlipStatusOK && twoGraysUnsolved(4,0)) //TT23-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(3,4) && !currentpieceFlipStatusOK && twoGraysUnsolved(0,1)) //TT26-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 2 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(3,4) && twoGraysUnsolved(0,1)) //TT30-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 2 && twoAdjacentPieces && twoSolvedPieces(2,3) && twoGraysUnsolved(4,0)) //TT33-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + lastSolvedPiece]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 3 && solvedCount == 1 && piecesSolvedStrict[4] && allCornersAlreadySolved && twoGraysUnsolved(1,2)) //TT36-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 1 && solvedCount == 0 && allCornersAlreadySolved && allEdgeColorsSolved && pieceOrder[0] == 29) //TT44-2
        {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[33].algo, g_faceNeighbors[LIGHT_BLUE + offby + 2]); //3e+ #33 same. (F/L safe tho)
        }
        else if (offby == 4 && solvedCount == 0 && allCornersAlreadySolved && twoGraysUnsolved(3,4)) //TT47-2
        {
...
//HORSEFACE-
        ...
        else if (offby == 3 && solvedCount == 2 &&
        (twoSolvedPieces(0, 2) ||
         twoSolvedPieces(0, 3) ||   // && allEdgeColorsSolved)  //test6-p2 ->(blue/green)=PASS //RandomTestCube1-3
         twoSolvedPieces(4, 1)))   //TT-34-1
        {
            colordirs loc;
            if (piecesSolvedStrict[2])
                loc = g_faceNeighbors[LIGHT_BLUE];
            else if (piecesSolvedStrict[3])
                loc = g_faceNeighbors[PINK];
            else if (piecesSolvedStrict[4])
                loc = g_faceNeighbors[BEIGE];
            //5 iterations goes "+" and preserves corners, 1 iteration goes "-" and does not.
            if (allCornersAlreadySolved) {
        ...
        else if (offby == 3 && solvedCount == 2 && twoSolvedPieces(2, 4) && allCornersAlreadySolved) //TT-40-2
        {
...
//HORSEFACE+
        else if (solvedCount == 2 && piecesSolvedStrict[0]
            && ((offby == 2 && piecesSolvedStrict[2]) || (offby == 1 && piecesSolvedStrict[3]))
            && (allEdgeColorsSolved || twoGraysUnsolved(1, 2))  //testCube3-p2 ->(blue/green)=PASS  + //TT24-2
        ) {
            colordirs loc;
            if (piecesSolvedStrict[2])
                loc = g_faceNeighbors[LIGHT_BLUE];
        ...
        else if (solvedCount == 2 && allCornersAlreadySolved &&
              ((offby == 1 && twoSolvedPieces(2, 4)) //TT-39-2
            || (offby == 2 && twoSolvedPieces(1, 3))) //TT-41-2
        ) {
            bulk = shadowDom->ParseAlgorithmString(g_AlgoStrings[13].algo, g_faceNeighbors[LIGHT_BLUE + firstSolvedPiece]);    //algo #13 (3.2+ CW)
            for (int i = 0; i < g_AlgoStrings[13].repeatX; ++i)
                bulkShadowRotate(shadowDom, bulk);
            updateRotateQueueWithShadow(shadowDom);
            continue;
...
//Last Chance to do something. Solve off-colored blue.  //TT32-2
        else if (offby != 0 && !piecesSolvedStrict[0] && !currentpieceFlipStatusOK && !allCornersAlreadySolved) {
            shadowDom->shadowMultiRotate(GRAY, offby);
        }
        else
            unknownloop++;
