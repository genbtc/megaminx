...
void Megaminx::rotateSolve7thLayerCorners(Megaminx* shadowDom)
{
    ...
    do {
        ...
        {
            int offby_ = findIfFirstEdgeDefault - 25;
            shadowDom->shadowMultiRotate(GRAY, offby_);
            continue;
        }
        ...
        const pieceCheck1 checklist[59] = {
        ...
        for (pieceCheck1 query: checklist) {
            if (checkPieceMatches(pieceOrder, query.a, query.b, query.c, query.d, query.e)) {
                ...
                int faceIndex = (query.dir == Face::CCW) ? 0 : -1;
                colordirs loc = g_faceNeighbors[query.startface + faceIndex];
...
//DEV       std::cout << foundEdges[k] << ", ";
            if (foundEdges[k] >= 25 && foundEdges[k] <= 29)
                modby[foundEdges[k]-25]=newDifference[k];
        }
